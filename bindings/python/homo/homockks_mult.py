...
if __name__ == "__main__":
    ...
    ##Learn why these (honestly i thing random sampling is not as scary
    ##I think its just ordinary sampling for the coeff
    ...
    mult = encoder.encode(np.array([5,5]))
    print("enc_mult:", mult, "dec_mult:", encoder.decode(mult))
    c = ((1/scale) * poly.polymul(mult, poly.polyadd(enc_msg, Pub[0])),
         (1/scale) * poly.polymul(mult, Pub[1]))

    #------------------------------------------------------

    toAdd = encoder.encode([5,5])
    c = (poly.polyadd(toAdd, poly.polyadd(enc_msg, Pub[0])), Pub[1])
