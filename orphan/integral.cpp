...
integral_t *new_integral(unsigned long long absolute)
{
	...
	return new orphan::Integral((unsigned long long) absolute, 1);
}

integral_t *new_unsigned_integral(unsigned long long absolute)
{
	return new orphan::Integral(absolute, 0);
}
...
{
	orphan::Integral ret = (*lhs) + (*rhs);
	return new orphan::Integral(ret); //check std::move
}
...
{
	orphan::Integral ret = divRem(*lhs, *rhs);
	return new orphan::Integral(ret); //check std::move
}
