...
if __name__ == "__main__":
    ...
    q = 13 # z_q[x]/<x^4+1>
    Pub = (poly.polyadd(poly.polymul(-A, S) % q, E) % q, A)
    msg = z
    enc_msg = p
    print(Pub[0], "\n", Pub[1])

    c = (enc_msg + Pub[0], Pub[1])
    print("C_0:", c[0], "C_1:", c[1])

    decript = poly.polyadd(c[0], poly.polymul(c[1], S) % q) % q 
